...
cfg = {'MUSIC_EXTENSIONS': MUSIC_EXTENSIONS}


class Scanner:
    def __init__(self, music_path, database):
    ...
    def stop(self):
        if not self.stopping.is_set():
            logging.debug("Stopping the file scanner.")
            self.stopping.set()
        ...
        if self.updating.is_set():
            logging.debug("Timed out waiting for file scanner to stop.")
        else:
            self.stopping.clear()
        if not self.stopping.is_set():
            logging.debug("File scanner has stopped.")
    ...
    def update(self):
        ...
        """
        if self.updating.is_set():
            logging.warn("Library update requested, but already updating.")
        ...
        logging.info("Starting library update.")
    ...
    def _update(self):
        ...
        logging.debug("Added all new songs in %0.2f seconds." % finish_time)
        logging.info("Updated library in %0.2f seconds." % finish_time)
        self.build_views()

    def build_views(self):
        artist_names = self.database.distinct('artist')
        album_artists = self.database.distinct('albumartist')
        albums = []
        for aa in album_artists:
            logging.info('***** Artist: %s' % aa)
            album_titles = self.database.find({
                'albumartist': aa
            }).distinct('album')
            for title in album_titles:
                logging.info('     %s' % title)
                songs = self.database.find({'album': title, 'albumartist': aa})
                if not songs.count():
                    continue
                artists = songs.distinct('artist')
                genres = songs.distinct('genre')
                tracks = songs[0].get('totaltracks')
                try:
                    path = os.path.dirname(songs[0].get('location'))
                except:
                    path = ''
                date = songs[0]['date']
                album = {
                    ...
                    'location': path,
                    'date': date,
                    'albumartist': songs[0].get('albumartist'),
                    'compilation': songs[0].get('compilation')
                }
                albums.append(album)
        logging.info(albums)
        albums_db = self.database._database.albums
        albums_db.drop()
        albums_db.insert_many(albums)
    ...
    def _scan(self):
        ...
        if self.read_queue.qsize() < 1:
            ...
            return False
        logging.debug("Queued %d songs to be read." % self.read_queue.qsize())
        return True
    ...
    def _process_read_queue(self):
        ...
        while self.read_queue.qsize() > 0 and not self.stopping.is_set():
            ...
            try:
                ...
                logging.error("Error processing read queue.")
            logging.debug("Processed %d items in %0.2f seconds" %
                         (queue_size - self.read_queue.qsize(),
        ...
        for song in records:
            ...
            if not (os.path.isfile(path) and
                ...
                if removed % 100 == 0:
                    ...
                    remove = []
                    logging.debug("Removed %d songs in %0.2f seconds." %
...
def read_metadata((location, id, mtime, revision)):
    ...
    try:
        ...
        logging.error("%s made Mutagen explode." % location)
        return None
...
def parse_wma(song, metadata):
    ...
    for tag, value in metadata.iteritems():
        try:
            ...
            # add it.
            try:
                logging.warn("Skipping unrecognized tag %s with data %s in \
                            file %s" % (tag, value[0], song['location']))
            except:
                logging.warn("Skipping unrecognized tag %s with un-printable \
